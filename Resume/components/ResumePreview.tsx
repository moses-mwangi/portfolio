...
const defaultResumeData: ResumeData = {
  personalInfo: {
    ...
    summary: "Experienced software developer with a passion for creating innovative solutions and leading teams to success.",
  },
  experience: [
    {
      id: "1",
...
export function ResumePreview({ resumeData = defaultResumeData }: ResumePreviewProps) {
  ...
  return (
    <div className="space-y-6">
            ...
            >
              ...
              <div className="space-y-4">
                {resumeData.education.map((edu) => (
                  <div key={edu.id} className="bg-slate-50 dark:bg-slate-700 rounded-lg p-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="text-lg font-semibold">{edu.degree}</h3>
                        <p className="text-purple-600 dark:text-purple-400 font-medium">{edu.institution}</p>
                        <p className="text-sm text-slate-600 dark:text-slate-400">{edu.field}</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {Object.entries(
                  ...
                  <div key={category} className="bg-slate-50 dark:bg-slate-700 rounded-lg p-4">
                    <h3 className="font-semibold mb-3 text-green-600 dark:text-green-400">{category}</h3>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {resumeData.projects.map((project) => (
                  <div key={project.id} className="bg-slate-50 dark:bg-slate-700 rounded-lg p-4">
                    <h3 className="font-semibold text-orange-600 dark:text-orange-400">{project.name}</h3>
                    <p className="text-sm text-slate-600 dark:text-slate-400 mb-2">{project.description}</p>
                    <div className="flex flex-wrap gap-1 mb-2">
                      {project.technologies.map((tech, i) => (
                        <span key={i} className="text-xs bg-orange-100 dark:bg-orange-900 text-orange-800 dark:text-orange-200 px-2 py-1 rounded">
                          {tech}
                        </span>
                      ))}
                    </div>
      ...
      <div className="text-center text-sm text-slate-500 dark:text-slate-400">
        <p>This is a preview of your resume. Use the download button to generate a PDF.</p>
      </div>
    </div>
  );
}
