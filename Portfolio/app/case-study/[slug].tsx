...
export default function CaseStudyPage() {
  ...
  useEffect(() => {
    if (slug) {
      ...
      if (found?.relatedProjects) {
        const related = artifacts.filter(a => found.relatedProjects.includes(a.id));
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="text-center">
          ...
          <h1 className="text-2xl font-light">Loading case study...</h1>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <Head>
        <title>{study.title} — Case Study | Living Archive</title>
      ...
      <div style={{ backgroundColor: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
          ...
          <div className="max-w-5xl mx-auto px-6">
            ...
            >
              <div className="flex items-center gap-3 text-sm mb-6">
                <span className="px-3 py-1 rounded-full text-xs" style={{ backgroundColor: '#D46A4A20', color: '#D46A4A' }}>
                  Case Study
                </span>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-12 pt-8 border-t" style={{ borderColor: 'var(--border-color)' }}>
                <div>
                  <div className="text-xs font-mono opacity-40 mb-1">Role</div>
                  <div className="text-sm">{study.role}</div>
                </div>
                <div>
                  <div className="text-xs font-mono opacity-40 mb-1">Client</div>
                  <div className="text-sm">{study.client}</div>
                </div>
                <div>
                  <div className="text-xs font-mono opacity-40 mb-1">Team</div>
                  <div className="text-sm">{study.team}</div>
                </div>
                <div>
                  <div className="text-xs font-mono opacity-40 mb-1">Timeline</div>
        ...
        <div className="max-w-6xl mx-auto px-6 mb-20">
          <div className="rounded-xl overflow-hidden" style={{ backgroundColor: 'var(--bg-secondary)' }}>
            <div className="aspect-video relative">
              ...
              <div className="w-full h-full flex items-center justify-center" style={{ backgroundColor: '#D46A4A20' }}>
        ...
        <div className="max-w-5xl mx-auto px-6 mb-24">
          <div className="grid md:grid-cols-2 gap-12">
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <div className="w-12 h-0.5 mb-6" style={{ backgroundColor: '#D46A4A' }} />
            ...
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <div className="w-12 h-0.5 mb-6" style={{ backgroundColor: '#E8C468' }} />
        ...
        {/* Process Deep Dive */}
        <div className="bg-opacity-30 py-20 mb-20" style={{ backgroundColor: 'var(--bg-secondary)' }}>
          <div className="max-w-5xl mx-auto px-6">
            <div className="text-center mb-12">
              <h2 className="text-3xl font-light mb-3">The Process</h2>
              <p className="opacity-60">How we got from problem to solution</p>
            </div>
              ...
              {study.process.map((phase, idx) => (
                ...
                >
                  <div className="flex items-center gap-4 mb-6">
                    <div className="w-12 h-12 rounded-full flex items-center justify-center text-xl" style={{ backgroundColor: '#D46A4A20', color: '#D46A4A' }}>
                      {idx + 1}
                    </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-8 ml-4 md:ml-16">
                    <div>
                      <h4 className="text-sm font-mono opacity-50 mb-3">Key Steps</h4>
                    ...
                    <div className="p-4 rounded-lg" style={{ backgroundColor: 'var(--bg-primary)' }}>
                      <h4 className="text-sm font-mono opacity-50 mb-2">Key Insight</h4>
                      <p className="text-sm italic opacity-80">{phase.insights}</p>
                    </div>
                  </div>
                </motion.div>
              ))}
        ...
        <div className="max-w-5xl mx-auto px-6 mb-20">
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-12">
            {Object.entries(study.metrics).map(([key, value]) => (
              <div key={key} className="text-center p-6 rounded-lg" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                <div className="text-3xl font-light mb-2">{value}</div>
                <div className="text-xs font-mono opacity-50 uppercase">{key}</div>
              </div>
            ))}
          </div>
        ...
        {study.testimonial && (
          <div className="max-w-4xl mx-auto px-6 mb-20">
            <div className="p-8 rounded-xl text-center" style={{ backgroundColor: 'var(--bg-secondary)' }}>
              ...
              <p className="text-xl italic leading-relaxed">{study.testimonial.quote}</p>
              <div className="mt-6">
                <div className="font-medium">{study.testimonial.author}</div>
                <div className="text-sm opacity-50">{study.testimonial.role}</div>
              </div>
            </div>
          </div>
        )}
        ...
        {/* Technologies */}
        <div className="max-w-5xl mx-auto px-6 mb-20">
          <h3 className="text-sm font-mono opacity-50 mb-4 text-center">Technologies Used</h3>
          <div className="flex flex-wrap justify-center gap-3">
            {study.technologies.map(tech => (
              <span key={tech} className="px-4 py-2 rounded-full text-sm" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                {tech}
              </span>
            ))}
          </div>
        ...
        {relatedProjects.length > 0 && (
          <div className="max-w-6xl mx-auto px-6 mb-20">
            <div className="text-center mb-8">
              ...
              <p className="opacity-50 text-sm">Projects that share similar themes or techniques</p>
            </div>
            <div className="grid md:grid-cols-3 gap-6">
              {relatedProjects.map(project => (
                ...
                >
                  <div className="text-sm opacity-50 mb-2">{project.category}</div>
        ...
        <div className="max-w-5xl mx-auto px-6 mb-20">
          <div className="flex flex-wrap justify-center gap-4">
            ...
            {study.links.caseStudyPDF && (
              <a href={study.links.caseStudyPDF} download
                className="px-6 py-3 rounded-full text-sm border transition hover:opacity-70"
                style={{ borderColor: 'var(--border-color)' }}>
                Download PDF ↓
              </a>
